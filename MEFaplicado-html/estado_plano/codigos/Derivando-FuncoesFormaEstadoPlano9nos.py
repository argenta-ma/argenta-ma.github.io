...
dN1rr = sp.diff(N1, r, r)#.subs({r: r1, s: s1})
dN2rr = sp.diff(N2, r, r)#.subs({r: r2, s: s2})
dN3rr = sp.diff(N3, r, r)#.subs({r: r3, s: s3})
dN4rr = sp.diff(N4, r, r)#.subs({r: r4, s: s4})
dN5rr = sp.diff(N5, r, r)#.subs({r: r5, s: s5})
dN6rr = sp.diff(N6, r, r)#.subs({r: r6, s: s6})
dN7rr = sp.diff(N7, r, r)#.subs({r: r7, s: s7})
dN8rr = sp.diff(N8, r, r)#.subs({r: r8, s: s8})
dN9rr = sp.diff(N9, r, r)#.subs({r: r9, s: s9})
...
dN1ss = sp.diff(N1, s, s)#.subs({s: s1, s: s1})
dN2ss = sp.diff(N2, s, s)#.subs({s: s2, s: s2})
dN3ss = sp.diff(N3, s, s)#.subs({s: s3, s: s3})
dN4ss = sp.diff(N4, s, s)#.subs({s: s4, s: s4})
dN5ss = sp.diff(N5, s, s)#.subs({s: s5, s: s5})
dN6ss = sp.diff(N6, s, s)#.subs({s: s6, s: s6})
dN7ss = sp.diff(N7, s, s)#.subs({s: s7, s: s7})
dN8ss = sp.diff(N8, s, s)#.subs({s: s8, s: s8})
dN9ss = sp.diff(N9, s, s)#.subs({s: s9, s: s9})
#convertendo para função lambda nuympy
...
#derivada em r e s
dN1rs = sp.diff(N1, r, s)#.subs({r: r1, s: s1})
dN2rs = sp.diff(N2, r, s)#.subs({r: r2, s: s2})
dN3rs = sp.diff(N3, r, s)#.subs({r: r3, s: s3})
dN4rs = sp.diff(N4, r, s)#.subs({r: r4, s: s4})
dN5rs = sp.diff(N5, r, s)#.subs({r: r5, s: s5})
dN6rs = sp.diff(N6, r, s)#.subs({r: r6, s: s6})
dN7rs = sp.diff(N7, r, s)#.subs({r: r7, s: s7})
dN8rs = sp.diff(N8, r, s)#.subs({r: r8, s: s8})
dN9rs = sp.diff(N9, r, s)#.subs({r: r9, s: s9})
...
dNds2 = sp.Matrix([[dN1rr, dN1ss], [dN2rr, dN2ss], [dN3rr, dN3ss], [dN4rr, dN4ss], [dN5rr, dN5ss], [dN6rr, dN6ss], [dN7rr, dN7ss], [dN8rr, dN8ss], [dN9rr, dN9ss]])
dNdrs = sp.Matrix([dN1rs, dN2rs, dN3rs, dN4rs, dN5rs, dN6rs, dN7rs, dN8rs, dN9rs])
##Jacobiano analítico
#J = Xe.T * dNds
#JI = J.inv()
#
