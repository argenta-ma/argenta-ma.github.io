...
Usando 5 pontos para obter uma interpolação quarquica e depois linkando com a condição
...
FUNCIONANDO!!!!!!! 
Deslocamentos e rotações OK!
Aumenta a malha o valor converge!!! OK!!!

Momentos OK!!!

Cortes estão zerando com 1 elemento!!!?? Com mais elementos são constantes!
Precisa de 100 elementos para aproximar o valor do corte!!!

CORTES FUNCIONAM QAUNDO SÃO CALCULADOS PELA DERIVADA DOS MOMENTOS!!!

Com 1 elemento apenas reproduz Euler-Bernouilli, porque zera o gamma_xz!!
...
r1 = -L*sp.Rational(1, 4) #objetivo: eliminar esse pelo link
r2 = 0
r3 = L*sp.Rational(1, 4) #objetivo: eliminar esse pelo link
r4 = L*sp.Rational(1, 2)
...
Mat_Coef = sp.Matrix([[1, r0, r0**2, r0**3, r0**4],
                      [1, r1, r1**2, r1**3, r1**4],
                      [1, r2, r2**2, r2**3, r2**4],
                      [1, r3, r3**2, r3**3, r3**4],
                      [1, r4, r4**2, r4**3, r4**4]])

U = sp.Matrix([u0, u1, u2, u3, u4])
...
Ns = sp.expand(A + B*r + C*r**2 + D*r**3 + E*r**4)
...
# plt.legend(bbor_to_anchor=(1.05, 1), loc=2, borderarespad=0.)
# plt.show()
...
## !!!!!! ATENÇÃO!!!!! multipliando por 2/L para simular o elemento padrão, pois partiu-se do elemento entre -1 e +1. ????
#retirando para ver... fazendo no início de -L/2 a L/2
...
#isolar das equações acima, w1, w3 e theta1, theta3 para resolver Ax = B
incognitas = sp.Matrix([theta1, theta3, u1, u3])

incognitas = sp.solve([gamma_xz_linear, gamma_xz_quad, gamma_xz_cub, gamma_xz_quar], [theta1, theta3, u1, u3])
...
wLinked = sp.expand(w.subs({u1:incognitas[u1], u3:incognitas[u3]}))
thetaLinked = sp.expand(theta.subs({theta1:incognitas[theta1], theta3:incognitas[theta3]}))


# #usando o theta2c_subs
# wLinked = sp.expand(w.subs({u1:incognitas[u1], u3:incognitas[u3], theta2: theta2c_subs}))
# thetaLinked = sp.expand(theta.subs({theta1:incognitas[theta1], theta3:incognitas[theta3], theta2: theta2c_subs}))

####!!!!! ATÉ AQUI, MESMA METOGOLODIA DO ONATE PARA 2 NÓS, exceto pela integração que será analítica de -L/2 até L/2
...
### gama_xz3 não existe aqui! --- Dá na mesma!!!!! Substituir o theta2 aqui?!?!?
# gamma_xz3 = sp.expand(gamma_xz.subs({theta1:incognitas[theta1], theta3:incognitas[theta3], u1:incognitas[u1], u3:incognitas[u3]}))


#usando o theta2c_subs
# gamma_xz3 = sp.expand(gamma_xz.subs({theta1:incognitas[theta1], theta3:incognitas[theta3], u1:incognitas[u1], u3:incognitas[u3], theta2: theta2c_subs}))
...
wNu4 = sp.Add(*[argi for argi in wLinked.args if argi.has(u4)]).subs(u4, 1)
wNtheta0 = sp.Add(*[argi for argi in wLinked.args if argi.has(theta0)]).subs(theta0, 1)
wNtheta2 = sp.Add(*[argi for argi in wLinked.args if argi.has(theta2)]).subs(theta2, 1) # É IGUAL A ZERO!!! Ou seja, wLinked não é função de theta2!!!
wNtheta4 = sp.Add(*[argi for argi in wLinked.args if argi.has(theta4)]).subs(theta4, 1)
...
#nó 3
tB4 = sp.Add(*[argi for argi in dtheta_dr.args if argi.has(u4)]).subs(u4, 1)
tB5 = sp.Add(*[argi for argi in dtheta_dr.args if argi.has(theta4)]).subs(theta4, 1)
...
#nó 3
gB4 = sp.Add(*[argi for argi in gamma_xzLinked.args if argi.has(u4)]).subs(u4, 1)
gB5 = sp.Add(*[argi for argi in gamma_xzLinked.args if argi.has(theta4)]).subs(theta4, 1)
...
Nb = sp.Matrix([ wNu0, wNtheta0, wNu2, wNtheta2, wNu4, wNtheta4 ])
q = sp.Symbol('q')
